...
export class UtilityMethods {
    ...
    public bindNodeProperties(node: NodeModel, selectedItem: SelectorViewModel): void {
        ...
        (selectedItem as any).nodeProperties.boothData = (node.addInfo as any).boothData;
        (selectedItem as any).nodeProperties.tempData = (node.addInfo as any).tempData;
        // (selectedItem as any).nodeProperties.lockedStatus = (node.addInfo as any).lockedStatus;
        // (selectedItem as any).nodeProperties.relatedProducts = (node.addInfo as any).relatedProducts;
        // (selectedItem as any).nodeProperties.bindEnterprise = (node.addInfo as any).bindEnterprise;
        // (selectedItem as any).nodeProperties.boothHeight = (node.addInfo as any).boothHeight;
