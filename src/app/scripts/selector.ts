...
export class NodeProperties {
  ...
  //   this.m_size = this.width * this.height;
  //   return this.m_size;
  // }
  ...
  //   if (this.m_size !== size) {
  //     this.m_size = size || 3;
  //     this.triggerPropertyChange("nodeSize", size);
  //   }
  // }

  // 新增的展位高度字段
  // private m_boothHeight = 0;
  // public get boothHeight(): number {
  //   return this.m_boothHeight;
  // }

  // public set boothHeight(boothHeight: number) {
  //   if (this.m_boothHeight !== boothHeight) {
  //     this.m_boothHeight = boothHeight || 3;
  //     this.triggerPropertyChange("nodeBoothHeight", boothHeight);
  //   }
  // }

  //添加锁定状态文本属性
  // private m_lockedStatus = "";
  // public get lockedStatus(): string {
  //   return this.m_lockedStatus;
  // }

  // public set lockedStatus(lockedStatus: string) {
  //   if (this.m_lockedStatus !== lockedStatus) {
  //     this.m_lockedStatus = lockedStatus;
  //     this.triggerPropertyChange("nodeLockedStatus", lockedStatus);
  //   }
  // }

  //添加关联商品文本属性
  // private m_relatedProducts = "";
  // public get relatedProducts(): string {
  //   return this.m_relatedProducts;
  // }

  // public set relatedProducts(relatedProducts: string) {
  //   if (this.m_relatedProducts !== relatedProducts) {
  //     this.m_relatedProducts = relatedProducts;
  //     this.triggerPropertyChange("nodeRelatedProducts", relatedProducts);
  //   }
  // }

  //添加绑定企业文本属性
  // private m_bindEnterprise = "";
  // public get bindEnterprise(): string {
  //   return this.m_bindEnterprise;
  // }

  // public set bindEnterprise(bindEnterprise: string) {
  //   if (this.m_bindEnterprise !== bindEnterprise) {
  //     this.m_bindEnterprise = bindEnterprise;
  //     this.triggerPropertyChange("nodeBindEnterprise", bindEnterprise);
  //   }
  // }

  //添加展位号文本属性
  private m_boothData = "";
  public get boothData(): string {
    return this.m_boothData;
  }

  public set boothData(boothData: string) {
    if (this.m_boothData !== boothData) {
      this.m_boothData = boothData;
      this.triggerPropertyChange("nodeboothData", boothData);
    }
  }
  ...
  public set tempData(tempData: any) {
    if (this.m_tempData !== tempData) {
      this.m_tempData = tempData;
      this.triggerPropertyChange("nodetempData", tempData);
    }
  }
...
export class SelectorViewModel {
          ...
          for (let i: number = 0; i < (selectedNodes as any).length; i++) {
            ...
            switch (propertyName1) {
              ...
              case "aspectratio":
                ...
                break;
              case "nodeboothdata":
                (node.addInfo as any).boothData = this.nodeProperties.boothData;
                break;
              case "nodetempdata":
                (node.addInfo as any).tempData = this.nodeProperties.tempData;
                console.log("获取临时数据：", this.nodeProperties.tempData);
                const dom = document.createElement('div');
                 dom.innerHTML = `<b>展位属性<b><br/><br/>`
                for (let item of this.nodeProperties.tempData.tempfield) {
                  this.viewField = this.nodeProperties.tempData.fieldss.find((element :any) => item.fieldId == element.fieldId)
                  if (Object.keys(this.nodeProperties.tempData.selectedrow[0]).includes(item.vModel)) {
                    node.constraints =
                      (NodeConstraints.Default & ~NodeConstraints.InheritTooltip) |
                      NodeConstraints.Tooltip;
                    dom.innerHTML += `
                                  ${this.viewField.config.label}:&nbsp;&nbsp;${this.nodeProperties.tempData.selectedrow[0][this.viewField.vModel]}<br/>`;
                  }
                }
                node.tooltip = {
                  content: dom,
                ...
                break;
              // case "nodeboothheight":
              //   (node.addInfo as any).boothHeight = this.nodeProperties.boothHeight;
              //   break;
              // case "nodelockedstatus":
              //   (node.addInfo as any).lockedStatus = this.nodeProperties.lockedStatus;
              //   break;
              // case "noderelatedproducts":
              //   (node.addInfo as any).relatedProducts = this.nodeProperties.relatedProducts;
              //   break;
              // case "nodebindenterprise":
              //   (node.addInfo as any).bindEnterprise = this.nodeProperties.bindEnterprise;
              //   break;
            }
            if (!node.children) {
  ...
  private applyNodeStyle(propertyName: string, node: Node): void {
    ...
    switch (propertyName) {
      case "fillcolor":
        console.log(node.style.fill, "测试")
        console.log(this.nodeProperties.fillColor, "测试")
